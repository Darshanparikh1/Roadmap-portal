...
export function ResetPasswordPage() {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.details.password) setErrors({ password: err.details.password });
  ...
  return (
    <AuthCard description="Signing in with the new password ends all other sessions." title="Choose a new password">
      <Form className="flex flex-col gap-4" errors={errors} onSubmit={onSubmit}>
        ...
        <Button className="brand-button w-full" loading={pending} type="submit">
          Update password
        </Button>
      </Form>
    </AuthCard>
